      ...
      <svg width={size} height={size} style={{ position: "relative", display: "block" }}>
        <defs>
          <radialGradient id={`fill-${size}-${variant}`} cx="50%" cy="50%" r="50%">
            <stop offset="0%" stopColor="oklch(0.85 0.12 50)" stopOpacity="0.45" />
            <stop offset="100%" stopColor="oklch(0.72 0.16 38)" stopOpacity="0.20" />
          </radialGradient>
        </defs>
        ...
        {/* Main polygon */}
        <path d={polyPath} fill={`url(#fill-${size}-${variant})`} stroke="oklch(0.65 0.16 38)" strokeWidth="1.2" strokeLinejoin="round" />
        ...
        {values.map((v, i) => {
          ...
          return (
            <circle
              ...
              fill={isHi || isSpine ? "var(--accent)" : "oklch(0.55 0.14 38)"}
